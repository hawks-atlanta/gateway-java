...
public class ITShareListWhitWho {
    ...
    @Test void shareListWhitWho ()
    {
        ...
        assertEquals (200, CtrlShareListWithWho.share_list_with_who(reqFile).code, "List of shared with users successfully");

        // errors 
        reqFile.fileUUID = null;
        assertEquals (400, CtrlShareListWithWho.share_list_with_who(reqFile).code,"Bad Request: Invalid Field");

		reqFile.token = "invalid token";
        reqFile.fileUUID = resSaveFile1.fileUUID;
		assertEquals (401, CtrlShareListWithWho.share_list_with_who(reqFile).code, "Authorization failed");

        reqFile.fileUUID = UUID.randomUUID ();
        reqFile.token = tokenUser1;
        assertEquals (404, CtrlShareListWithWho.share_list_with_who(reqFile).code, "There is no file with the given fileUUID");

        reqFile.fileUUID = resSaveFile1.fileUUID;
        TestUtilConfig.makeInvalidMetadata ();
        assertEquals (500, CtrlShareListWithWho.share_list_with_who(reqFile).code, "Can't reach metadata");
