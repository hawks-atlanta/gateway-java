...
public class ITUpdatePassword
{
	...
	@Test void updatedPassword ()
	{
		...
		assertEquals (
			400, CtrlUpdatePassword.account_password (accPassword).code,
		...
		assertEquals (
			401, CtrlUpdatePassword.account_password (accPassword).code, "Authorization failed");

		accPassword.token = res.auth.token;
		assertEquals (
			401, CtrlUpdatePassword.account_password (accPassword).code, "Invalid credentials");

		accPassword.oldpassword = cred.password;
		accPassword.token = res.auth.token;
		assertEquals (
			200, CtrlUpdatePassword.account_password (accPassword).code,
			"Password updated successfully");

		TestUtilConfig.makeInvalidAll ();
		assertEquals (
			500, CtrlUpdatePassword.account_password (accPassword).code,
